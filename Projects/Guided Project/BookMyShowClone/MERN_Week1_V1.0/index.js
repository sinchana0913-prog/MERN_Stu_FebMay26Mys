...
bookingEmitter.on("bookingStarted", () => {
    console.log(chalk.blue("Booking proccess started."));
});
...
function handleInvalidAttempt(message) {
    ...
    if (invalidAttempts >= MAX_ATTEMPTS) {
        console.log(chalk.red("\nMax invalid attempts reached.Existing app."));
        rl.close();
        return true;
    }
    return false;
...
function showMenu() {
    ...
    console.log("1.View Movies |2.Book Tickets |3.ViewBooking");
        ...
        validateMovieSelection(movies, movieId, (movieError, selectedMovie) => {
            ...
            rl.question("Enter time slot as displayed:", (timeInput) => {
                validateTimeSelection(selectedMovie, timeInput, (timeError, selectedShowtime) => {
                    ...
                    rl.question("Enter number of seats:", async (seatInput) => {
                        const seatCount = Number(seatInput);
                        validateseatCount(seatCount, async (seatError, validateSeatCount) => {
                            if (seatError) {
